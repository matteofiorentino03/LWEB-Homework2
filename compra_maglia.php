...
$is_admin = (isset($user['ruolo']) && strtolower($user['ruolo'])==='admin');
$homepage_link = $is_admin ? 'homepage_admin.php' : 'homepage_user.php';

/* ===== Parametri maglia dal catalogo ===== */
...
/* ===== Varianti per taglia (stessa maglia, taglie diverse) ===== */
$sqlMaglie = "SELECT ID, tipo, taglia, Sponsor, stagione, descrizione_maglia, costo_fisso, path_immagine
              FROM Maglie WHERE tipo=? AND stagione=? ORDER BY FIELD(taglia,'S','M','L','XL')";
$stmt = $conn->prepare($sqlMaglie);
$stmt->bind_param("ss", $tipo, $stagione);
$stmt->execute();
$resMaglie = $stmt->get_result();
$varianti  = [];
while ($r = $resMaglie->fetch_assoc()) $varianti[] = $r;
$stmt->close();
...
/* ===== Lista giocatori per selezione (per Maglie_Giocatore) ===== */
$giocatori = [];
$qr = $conn->query("SELECT ID, nome, cognome, num_maglia FROM Giocatori ORDER BY cognome, nome");
while ($g = $qr->fetch_assoc()) $giocatori[] = $g;

/* ===== Gestione acquisto (POST conferma) ===== */
$msg = '';
$ok  = false;
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['azione']) && $_POST['azione']==='acquista') {

    $ID_Maglia_selezionata = (int)($_POST['maglia_id'] ?? 0);
    $taglia_sel            = $_POST['taglia'] ?? '';
    $indirizzo             = trim($_POST['indirizzo'] ?? '');
    $mod_tipo              = $_POST['mod_tipo'] ?? 'none'; // none|giocatore|personalizzata

    // Dati per supplemento
    $logo   = $_POST['logo'] ?? ''; // "" se non scelto
    $id_g   = (int)($_POST['id_giocatore'] ?? 0);
    $nome_p = trim($_POST['nome_pers'] ?? '');
    $num_p  = trim($_POST['num_pers'] ?? '');

    // Verifico maglia scelta
    $stmt = $conn->prepare("SELECT ID, costo_fisso FROM Maglie WHERE ID=? AND tipo=? AND stagione=? AND taglia=?");
    $stmt->bind_param("isss", $ID_Maglia_selezionata, $tipo, $stagione, $taglia_sel);
    $stmt->execute();
    $magliaRow = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    ...
    if (!$magliaRow) {
        ...
        // Calcolo supplemento lato server
        $supplemento = 0;
        ...
        if ($msg === '') {
            $base   = (float)$magliaRow['costo_fisso'];
            ...
            if ($crediti < $totale) {
                ...
                try {
                    // Inserisco in Compra
                    $oggi = date('Y-m-d');
                    $insCompra = $conn->prepare("INSERT INTO Compra (ID_Utente, ID_Maglia, pagamento_finale, indirizzo_consegna, data_compra)
                                                 VALUES (?,?,?,?,?)");
                    $insCompra->bind_param("iiiss", $userId, $ID_Maglia_selezionata, $totale, $indirizzo, $oggi);
                    $insCompra->execute();
                    $id_compra = $conn->insert_id;
                    $insCompra->close();

                    // Inserisco eventuale personalizzazione
                    if ($mod_tipo === 'giocatore') {
                        $insG = $conn->prepare("INSERT INTO Maglie_Giocatore (Supplemento, Logo, ID_Giocatore, ID_Maglia)
                                                VALUES (?,?,?,?)");
                        $lg = ($logo==='') ? NULL : $logo;
                        $insG->bind_param("isii", $supplemento, $lg, $id_g, $ID_Maglia_selezionata);
                        $insG->execute();
                        $insG->close();
                    } elseif ($mod_tipo === 'personalizzata') {
                        $insP = $conn->prepare("INSERT INTO Maglie_Personalizzate (ID_Maglia, Logo, supplemento, nome, num_maglia)
                                                VALUES (?,?,?,?,?)");
                        $lg = ($logo==='') ? NULL : $logo;
                        $numInt = (int)$num_p;
                        $insP->bind_param("isisi", $ID_Maglia_selezionata, $lg, $supplemento, $nome_p, $numInt);
                        $insP->execute();
                        $insP->close();
                    }
                    ...
                    $conn->rollback();
                    $msg = "Errore durante l'acquisto: ".$e->getMessage();
                }
            }
        }
    }
...
/* ===== Valori di comodo per il rendering ===== */
$prima = $varianti[0]; // per mostra iniziale
  ...
  <form method="post" id="acquistoForm">
    ...
    <div class="grid">
      ...
      <div class="details">
        ...
        <!-- Selezione tipo personalizzazione -->
        <div class="card">
          <div class="opt-row">
...
<script>
/* ===== Varianti: cambio taglia -> cambia ID, prezzo, immagine ===== */
